...
All images provided by client. Located in `/mnt/user-data/uploads/`:

| Filename | Description | Recommended Use |
|----------|-------------|-----------------|
| `UNLIMITED_JUNK_LOGO.png` | Logo (red bg, white icon/text) | Header, favicon source |
| `Unlimited_Junk_-_Dan_by_trailer.jpeg` | Dan standing with truck/trailer | About page, Contact page |
| `UJ_-_Garage_-_Before.JPG` | Cluttered garage (before) | Services page - cleanouts |
| `UJ_-_Garage_-_After.JPG` | Clean garage (after) | Services page - cleanouts |
| `UJ_-_Alley_-_Before_1.JPG` | Alley with debris (before) | Homepage or Services |
| `UJ_-_Alley_-_Before_2.JPG` | Alley with debris, alt angle | Backup/alternate |
| `UJ_-_Alley_-_After.JPG` | Clean alley (after) | Homepage or Services |

**Image processing notes for Claude Code:**
- Compress all images (WebP preferred, JPEG fallback)
- Resize to reasonable max widths (e.g., 1200px for full-width, 600px for cards)
- Create a version of the logo with transparent background if needed for header
- Generate favicon from logo icon

**Before/After pairing:**
- Garage: Before → After (dramatic transformation, good for cleanouts section)
- Alley: Before 1 or 2 → After (shows debris removal)

**Logo:** Emerson has high-res vector available if needed. Current PNG is sufficient for web use.

**Deployment approach:**
1. Build in GitHub repo
2. Deploy to Netlify preview URL (e.g., `unlimited-junk.netlify.app`)
3. Dan reviews live preview
4. Once approved, transfer domain from Wix and point to Netlify
...
3. **Final approval:** Dan confirms he wants to proceed with Netlify migration
